...
export function PosterEditor() {
  ...
  const handleQuickSave = useCallback(() => {
    const savedMaps = JSON.parse(localStorage.getItem('cartistry-saved-maps') || '[]');
    const newMap = {
    ...
    localStorage.setItem('cartistry-saved-maps', JSON.stringify(savedMaps.slice(0, 50)));
    setQuickSaved(true);
    ...
    <div className="flex flex-col md:flex-row h-screen bg-gray-50 dark:bg-gray-900 overflow-hidden">
      ...
      <div className="md:hidden h-14 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-4 z-40 shadow-sm">
        <Link href="/" className="flex items-center gap-2">
          <Logo size="lg" />
          <span className="font-bold text-gray-900 dark:text-white">Cartistry</span>
        </Link>
      ...
      >
        ...
        {/* Inspired By Credit - Bottom Right */}
        <a
          href="https://www.reddit.com/r/MapPorn/comments/1q0ql56/i_built_a_free_tool_to_create_custom_map_posters/"
          target="_blank"
          rel="noopener noreferrer"
          className="absolute bottom-4 right-4 z-40 hidden md:flex items-center gap-1.5 text-[10px] text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors opacity-60 hover:opacity-100"
        >
